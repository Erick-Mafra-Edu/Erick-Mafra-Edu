# 📚 Erick Marlon Mafra | Repositório de Estudos e Formação

Este é o meu espaço dedicado ao **aprendizado contínuo** e à **construção de uma base técnica sólida** para minha carreira como Desenvolvedor/DevOps.

Aqui são armazenados códigos de cursos, tutoriais, projetos de *bootcamps* e testes de novas tecnologias.

---

<table>
<tr>
<!-- Coluna 1: Imagem estilo ASCII Art -->
<td valign="top" width="50%">
  <img src="https://raw.githubusercontent.com/Erick-Mafra-Edu/Erick-Mafra-Edu/main/github.png" alt="Erick ASCII Art" width="100%">
</td>

<!-- Coluna 2: Informações estilo Neofetch -->
<td valign="top" width="50%">

**erick@github**
<hr>

* **OS**: Academic / Self-Taught Linux Environment
* **Host**: Univali (Ciência da Computação)
* **Kernel**: PHP, Node.js, Python
* **Infrastructure**: Azure, Docker
* **Monitoring**: Grafana, Zabbix
* **Architecture**: Cache & Message Queues (Filas)
* **Shell**: [Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Erick-Mafra-Edu&layout=compact&theme=transparent)

<br>

**🔗 CONTACT_LINKS**
<hr>

* [![Mail](https://img.shields.io/badge/iCloud-3693F3?style=for-the-badge&logo=iCloud&logoColor=white)](mailto:erick.mafra@icloud.com)
* [![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/erick-mafra/)
* [![Linkedin](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/erick-mafra-3726a4257/)

<br>

**🎨 THEME_COLORS**
<hr>
🔴 🟢 🟡 🔵 🟣 ⚫ ⚪

</td>
</tr>
</table>

---
<!--
Svg dos ultimos estudos farei depois
[![ErickMafstudys]()]
-->
